...
x_varobj=[(5*olds_fam[i]+10*kids_fam[i]+2*adults_fam[i]) for j in range(0,num_buildings) for i in range(num_families)]
...
Model.linear_constraints.add(lin_expr = [cplex.SparsePair(ind=[x_vars[i,j]],val=[building_distance[i][j]]) for i in range(num_families) for j in range(num_buildings)], 
                                        ...
                                        rhs = [200 for i in range(num_families) for j in range(num_buildings)])   
...
dispersion_gdf.to_file("C:\\Users\\ggalv\\Google Drive\\Respaldo\\TESIS MAGISTER\\Simulacion-evacuacion-antofagasta\\resultados_modelo_matematico\\shape_dist_ninos\\dist_ninos_edificios_maichel.shp")
...
x_varub = [1.0]*len(x_varnames)
# x_varobj=[(5*olds_fam[i]+3*kids_fam[i]+adults_fam[i])-(building_distance[i][j]/100) for j in range(num_buildings) for i in range(num_families)]
x_varobj=[(10*olds_fam[i]+5*kids_fam[i]+2*adults_fam[i]) for j in range(0,num_buildings) for i in range(num_families)]
...
dispersion_gdf.to_file("C:\\Users\\ggalv\\Google Drive\\Respaldo\\TESIS MAGISTER\\Simulacion-evacuacion-antofagasta\\resultados_modelo_matematico\\shape_dist_ninos\\dist_abuelos_edificios_maichel.shp")
...
for i in range(num_families):
    ...
    #         path[id_fams[i]]=[path_id,id_meatingpoints[k]]
#Guardar diccionario
np.save('C:\\Users\\ggalv\\Google Drive\\Respaldo\\TESIS MAGISTER\\Simulacion-evacuacion-antofagasta\\resultados_modelo_matematico\\scape_route_optimal_abuelos_primero_2.npy', path)
